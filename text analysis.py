...
Original file is located at
    https://colab.research.google.com/drive/1e5cxG6k3hsNk_Fb3RM-o2c7W4cj6yaNy
...
student_id = 2310825 # Note this is an interger and you need to input your id
...
def save_model(model, model_dir):
    ...
    model_file = os.path.join(model_dir, 'logistic_regression_model.pkl')  # Corrected model file path
...
def train_Gen(train_file, val_file, model_dir):
    ...
    train_df['comment'] = train_df['comment'].str.replace(r'SDATA_5 : ', '').str.replace(r' : EDATA_5', '')
    valid_df['comment'] = valid_df['comment'].str.replace(r'SDATA_5 : ', '').str.replace(r' : EDATA_5', '')
    ...
    history = model.fit(padded_sequences, train_df['toxicity'], epochs=1, verbose=1)

    # Tokenize and pad the validation data
    valid_sequences = tokenizer.texts_to_sequences(valid_df['comment'])
    padded_valid_sequences = pad_sequences(valid_sequences, maxlen=max_seq_length, padding='post')

    # Predict on the validation data
    predictions = model.predict(padded_valid_sequences)
...
# Calculate accuracy
    accuracy = np.mean(binary_predictions.flatten() == valid_df['toxicity'])
...
def test_Gen(test_file, MODEL_PATH,model_gdrive_link):
    ...
    # Temporary directory and file for model
    ...
    # Load test data
    print('\nLoad test data from', test_file)
    test_df = pd.read_csv(test_file)

    # Load the model file
    model_file_path = os.path.join(test_model_path, 'lstm_model.h5')
    model = tf.keras.models.load_model(model_file_path)

    # Tokenize and pad the test data
    tokenizer = Tokenizer()
    tokenizer.fit_on_texts(test_df['comment'])
    ...
    test_df['predicted_toxicity'] = binary_predictions

    # Save the test dataframe with predictions
    output_file = os.path.join(MODEL_PATH, 'test_results.csv')
    test_df.to_csv(output_file, index=False)
    print('\nTest results saved in', output_file)
...
words_stopwords = set(stopwords.words('english'))
def remove_stopwords(tokens):
    return [word for word in tokens if word.lower() not in words_stopwords]
...
def train_dis(train_file, val_file, model_dir):
    ...
    # Predict on the validation data
    y_pred = logreg.predict(X_valid_counts)
    ...
    MODEL_Dis_File = os.path.join(model_dir, 'Model_Dis.zip')
    ...
    # Get a shareable link for the model
...
def test_dis(test_file, MODEL_PATH,model_gdrive_link):
    ...
    # Preprocess the test data (assuming the same preprocessing as training data)
    test_df['comment'] = test_df['comment'].str.replace(r'SDATA_5 : ', '').str.replace(r' : EDATA_5', '')

    # Load the saved model
    model_file = os.path.join(test_model_path, 'logistic_regression_model.pkl')
    model = load_model(model_file)

    # Vectorize the test data
    X_test_counts = vectorizer.transform(test_df['comment'])
    ...
    print("Confusion Matrix:")
    print(confusion_matrix(test_df['toxicity'], y_pred))
